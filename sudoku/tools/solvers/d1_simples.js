// Verificación de los 6 puzzles «solo dígitos» del lote D1.
'use strict';
...
function reporte(nombre, sols, esperada) {
  const n = sols.length;
  let linea = `${nombre}: ${n === 0 ? 'SIN SOLUCION' : n === 1 ? 'UNICA' : n + '+ SOLUCIONES'}`;
  if (n >= 1) {
    linea += ' | ' + fmt(sols[0]);
...
// ---------- Dancing in a ring (b37xfe4xjf) ----------
(function () {
  const units = [...rowsCols(9), ...boxesRect(9, 3, 3)];
  const pairs = [];
  const whLoop = cells => { // whisper cíclico
  ...
  const s = makeSolver({ N: 9, digits: D9, units, pairs, groups });
  reporte('Dancing in a ring', s.solve(2),
...
(function () {
  ...
  const s = makeSolver({ N: 9, digits: D9, units, groups, givens });
  reporte('Overlap', s.solve(2),
    '862493157973215468154687932647932581318756249295841376781324695539168724426579813');
})();

// ---------- The Dutch Miracle (hfpFTGNLrr) ----------
(function () {
  const units = [...rowsCols(9), ...boxesRect(9, 3, 3)];
  // diagonales positivas (SW->NE): r+c constante; sin repetir + adyacentes difieren >= 4
  const pairs = [];
  for (let s0 = 0; s0 <= 16; s0++) {
    const diag = [];
    for (let r = 0; r < 9; r++) { const c = s0 - r; if (c >= 0 && c < 9) diag.push(I(r, c)); }
    if (diag.length >= 2) {
      units.push(diag);
  ...
  reporte('Dutch Miracle', s.solve(2), null);
})();

// ---------- Renban ModE (HhQT427HGN) ----------
(function () {
  const units = [...rowsCols(9), ...boxesRect(9, 3, 3)];
  const R = cells => renban(cells.map(x => I(...x)));
  const groups = [
  ...
  const s = makeSolver({ N: 9, digits: D9, units, groups });
  reporte('Renban ModE', s.solve(2), null);
})();

// ---------- Difference Of Squares (8HQfQ4tD9q) ----------
(function () {
  ...
  const arrowSum = (bulb, cells) => ({
    ...
    partial(vals) {
      ...
      if (b === null) return sum + empty * 1 <= 9;
      return sum + empty * 1 <= b && b <= sum + empty * 9;
    }
  });
  const todas = [];
  for (let X = 4; X <= 9; X++) for (let Y = 3; Y < X && Y <= 7; Y++) {
    if (X - Y < 1 || X - Y > 9) continue;
    const S = (cells, v) => sumExact(cells.map(x => I(...x)), v, 1, 9);
    const groups = [
      S([[6,0],[6,1],[6,2]], 15),
      S([[0,8],[1,8]], X),
    ...
    const sols = s.solve(2);
    sols.forEach(x => todas.push({ X, Y, sol: fmt(x) }));
  }
  const n = todas.length;
  let linea = `Difference Of Squares: ${n === 0 ? 'SIN SOLUCION' : n === 1 ? 'UNICA' : n + '+ SOLUCIONES'}`;
  if (n >= 1) linea += ` | X=${todas[0].X} Y=${todas[0].Y} | ` + todas[0].sol +
    (todas[0].sol === esperada ? ' | == metadata OK' : ' | != METADATA!!');
  console.log(linea);
})();

// ---------- Hypnotic Suggestion (qt0xjevecl) ----------
(function () {
  ...
  const cage0 = p.cages.filter(c => c.cells && c.cells.length)[0];
  const cage1 = p.cages.filter(c => c.cells && c.cells.length)[1];
  // 87654321: ocho 7s, seis 5s, cuatro 3s, dos 1s / 12345678: un 2, tres 4s, cinco 6s, siete 8s
  const groups = [
    exactCounts(cage0.cells.map(x => I8(...x)), { 7: 8, 5: 6, 3: 4, 1: 2 }),
    exactCounts(cage1.cells.map(x => I8(...x)), { 2: 1, 4: 3, 6: 5, 8: 7 })
  ];
  const s = makeSolver({ N: 8, digits: D8, units, groups });
  reporte('Hypnotic Suggestion', s.solve(2),
