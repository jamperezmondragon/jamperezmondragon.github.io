...
for (const s of shadings) {
  const sols = solveDigits(s, 10);
  for (const g of sols) all.push({ sh: s.join(""), g: g.join("") });
}
console.log("soluciones totales:", all.length);
const shSet = new Set(all.map(a => a.sh));
console.log("sombreados distintos entre soluciones:", shSet.size);
for (const a of all) console.log(a.g, "|", a.sh);
