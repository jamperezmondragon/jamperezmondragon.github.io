...
for (const id of Object.keys(CFG)) {
  ...
  const r = analyze(id);
  console.log("=== " + r.name + " (" + id + ") " + r.N + "x" + r.N);
  console.log("  sudoku+estáticos:", r.errs.length ? "ERRORES: " + r.errs.join("; ") : "OK");
  console.log("  paredes(aristas):", r.walls, "| puertas:", r.doors, "| grosellas:", r.currants, "| esquinas:", r.corners);
  console.log("  rata:", JSON.stringify(r.rat), "pastel:", JSON.stringify(r.cake));
  console.log("  CAMINOS VÁLIDOS:", r.pathCount, "(nodos explorados:", r.nodes + ")");
  if (r.pathCount >= 1 && r.pathCount <= 3 && r.path)
    console.log("  camino:", r.path.map(([a, b]) => "r" + (a + 1) + "c" + (b + 1)).join("→"));
}
